...
pub struct CgalKdTreeWrapper<'a, const D: usize> {
    pub positions: Vec<DVec<D>>,
    pub graph: &'a crate::graph::Graph,
    index: *mut CgalKdTreeIndex,
    _phantom: PhantomData<&'a ()>,
}

impl<'a, const D: usize> Clone for CgalKdTreeWrapper<'a, D> {
    fn clone(&self) -> Self {
        Self::new(&Embedding {
...
impl<'a, const D: usize> CgalKdTreeWrapper<'a, D> {
    ...
    pub fn point_count(&self) -> usize {
        if self.index.is_null() {
            ...
            unsafe { cgal_kdtree_point_count(self.index) }
        }
    }
    ...
    pub fn dimensions(&self) -> usize {
        if self.index.is_null() {
            ...
            unsafe { cgal_kdtree_dimensions(self.index) }
        }
    }
}

impl<'a, const D: usize> Drop for CgalKdTreeWrapper<'a, D> {
    fn drop(&mut self) {
        if !self.index.is_null() {
            unsafe {
                cgal_kdtree_destroy_index(self.index);
            }
...
// Implement Send and Sync - CGAL queries are thread-safe for read-only operations
unsafe impl<'a, const D: usize> Send for CgalKdTreeWrapper<'a, D> {}
unsafe impl<'a, const D: usize> Sync for CgalKdTreeWrapper<'a, D> {}

impl<'a, const D: usize> Graph for CgalKdTreeWrapper<'a, D> {
...
impl<'a, const D: usize> Position<D> for CgalKdTreeWrapper<'a, D> {
...
impl<'a, const D: usize> Update<D> for CgalKdTreeWrapper<'a, D> {
...
impl<'a, const D: usize> Query for CgalKdTreeWrapper<'a, D> {
    fn nearest_neighbors(&self, index: usize, radius: f64, results: &mut Vec<NodeId>) {
        ...
        // Use epsilon=0.0 for exact radius search
        let epsilon = 0.0f32;

        // Perform radius search - returns dynamically allocated results
        let mut search_result = unsafe {
            cgal_kdtree_radius_search(self.index, query_point.as_ptr(), scaled_radius_squared, epsilon)
        };
        ...
        if !search_result.indices.is_null() && search_result.count > 0 {
            ...
            unsafe {
                cgal_kdtree_free_result(&mut search_result);
            }
        }
    }
}

impl<'a, const D: usize> SpatialIndex<D> for CgalKdTreeWrapper<'a, D> {
    fn name(&self) -> String {
        "cgal_kdtree".to_string()
    }

    fn implementation_string(&self) -> &'static str {
        include_str!("cgal_kdtree.rs")
    }
}

impl<'a, const D: usize> query::Embedder<'a, D> for CgalKdTreeWrapper<'a, D> {
