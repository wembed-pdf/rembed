...
fn main() -> io::Result<()> {
    ...
    println!("D, normal, reduction_radius, reduction_snn, reduction_both, snn_best, both_best");
...
fn embedd_and_calc_stats<const D: usize>(graph: &graph::Graph) {
    ...
    for i in 0..pos.len() {
        ...
        let params = QueryParams::new(false, false, false, false);
        analysis.query(i, 1., &mut results, &mut stats_normal, &params);
        let params = QueryParams::new(true, false, false, false);
        analysis.query(i, 1., &mut results, &mut stats_reduced_radius, &params);
        let params = QueryParams::new(false, true, false, false);
        analysis.query(i, 1., &mut results, &mut stats_snn, &params);
        let params = QueryParams::new(true, true, false, false);
        analysis.query(i, 1., &mut results, &mut stats_both, &params);
        let params = QueryParams::new(false, true, true, false);
        analysis.query(i, 1., &mut results, &mut stats_snn_best_dim, &params);
        let params = QueryParams::new(true, true, true, false);
        analysis.query(i, 1., &mut results, &mut stats_both_best_dim, &params);
    }
    println!(
        "{D}, {normal}, {reduction_radius}, {reduction_snn}, {reduction_both}, {snn_best}, {both_best}",
        D = D,
