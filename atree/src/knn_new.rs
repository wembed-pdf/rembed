    ...
    PDVec<D, W, F, I>: CompressDispatch<W, F, I>,
{
        ...
        (usize, F): QueryOutput<I, F>,
    {
        ...
        let mut results = self.nearest_n(pos, 1);
        results.pop()
    }
    ...
    pub fn nearest_n(&self, pos: &[F; D], k: usize) -> Vec<Neighbour<F>>
    where
        (usize, F): QueryOutput<I, F>,
    {
        self.nearest_n_core(pos, k, KnnHeap::new(k))
    }
    ...
    /// distance or `radius²`.
    pub fn nearest_n_within(&self, pos: &[F; D], k: usize, radius: F) -> Vec<Neighbour<F>>
    where
        (usize, F): QueryOutput<I, F>,
    {
        self.nearest_n_core(pos, k, KnnHeap::with_radius(k, radius * radius))
    }

    fn nearest_n_core(&self, pos: &[F; D], k: usize, mut heap: KnnHeap<F>) -> Vec<Neighbour<F>>
    where
        (usize, F): QueryOutput<I, F>,
    {
    ...
    fn scan_leaf_full(&self, pos: &Point<D, F>, k: usize, leaf_idx: usize, heap: &mut KnnHeap<F>)
    where
        (usize, F): QueryOutput<I, F>,
    {
        ...
        // }
        // let mut heap = KnnHeap::from_vec(vec, k, heap.max_dist);

        // for pdvec in &self.positions_sorted[vec_end..end] {
        for pdvec in &self.positions_sorted[start..end] {
            let dists = pdvec.dist_squared(pos.pos);
...
mod tests {
    ...
    fn nearest_n_matches_brute_force_2d() {
        ...
        for k in [1, 5, 10, 50] {
            ...
            for &qi in &[0usize, 100, 999, 4999] {
                ...
                let knn = tree.nearest_n(&query, k);
    ...
    fn empty_tree_returns_none() {
        ...
        assert!(tree.nearest_n(&[0.0, 0.0], 5).is_empty());
    }
    ...
    fn k_larger_than_n() {
        ...
        let tree: ATree<2> = ATree::new(&positions);
        let results = tree.nearest_n(&[0.0, 0.0], 100);
    ...
    fn nearest_n_within_respects_radius() {
        ...
        for radius in [0.05f32, 0.1, 0.2] {
            ...
            for &qi in &[0usize, 100, 2500] {
                ...
                let knn = tree.nearest_n_within(&query, k, radius);
